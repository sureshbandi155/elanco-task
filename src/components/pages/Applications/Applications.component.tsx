...
export const Applications: React.FC = () => {
  ...
  const [applicationOptions, setApplicationOptions] = useState<Options[]>([]);
  const [data, setData] = useState<Data[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE_API}/applications`)
      .then((res) => {
  ...
  const handleSearch = (e: any) => {
    ...
    if (applicationName.length) {
      axios
        .get(
          `${process.env.REACT_APP_BASE_API}/applications/${applicationName}`
        )
        .then((res) => {
